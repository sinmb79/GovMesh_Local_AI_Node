...
1. 정식 사용자/기관 계정 체계
   - 현재는 로컬 API 토큰 기반입니다.
   - 다음 단계는 기관망 SSO, mTLS, 단말 인증서, 사용자별 권한 정책으로 확장해야 합니다.

2. 전문 파일 검사
   - 현재 Quarantine Gateway는 경량 정적 검사입니다.
   - 실제 제출급 환경에서는 AV/YARA/CDR, 문서 매크로 검사, SBOM/서명 검증, 모델 파일 포맷 검증이 필요합니다.

3. Presidio 수준 PII 인식기
   - 현재는 정규식 기반 스캐너에 confidence/hash를 보강한 상태입니다.
   - 한국 행정 문서용 recognizer, checksum, 주변 문맥 점수, 오탐 리뷰 corpus가 필요합니다.

4. 근거성 평가와 환각 방지
   - RAG context ID는 유지되지만, 답변 문장별 citation 검증은 아직 없습니다.
   - 다음 단계는 contextual grounding score, 근거 없는 문장 차단, human review 큐입니다.

5. 감사 로그 운영 보관
   - HMAC 서명은 위변조 탐지력을 높이지만, 키 관리와 외부 보관까지 해결하지는 않습니다.
   - 운영 단계에서는 WORM 저장소, 주기적 checkpoint export, 외부 감사 보관소가 필요합니다.

6. Windows 설치 UX
   - 현재는 CLI와 서비스 스크립트 중심입니다.
   - 비개발자 운영자를 위해 설치 마법사, 상태 점검, 토큰 생성, 장애 진단 화면을 추가해야 합니다.
